...
class FirstSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        with open(Path(".\\"+self.settings.get('TWITTER_USERNAME')+"\\items.json"), 'r',encoding="utf-8") as f:
            items = f.readlines()
            for line in items:
                data = json.loads(line)
                print(data)
                if data.get("is_my_tweet") and data["is_reply"]:
                    yield scrapy.Request("https://nitter.it"+data["link"])
    ...
    def parse(self, response):
        result = {
            "html": response.css('div .conversation').get(),
            "link": response.request.url
        }
        if self.get_videos:
            result["file_urls"] = list(map(response.urljoin, response.css('source::attr(src)').getall()))
            result["file_urls"] += list(map(response.urljoin, response.css('video::attr(data-url)').getall()))
        if self.get_images:
            if self.only_pfps:
                result["image_urls"] = list(map(response.urljoin, response.css("img.avatar::attr(src)").getall()))
            else:
                result["image_urls"] = list(map(response.urljoin, response.css("img::attr(src)").getall()))
                result["image_urls"] += list(map(response.urljoin, response.css("video::attr(poster)").getall()))
                # if result["is_my_tweet"]:
                result["image_urls"] += list(map(response.urljoin, response.css("a.still-image::attr(href)").getall()))
        yield result
